...
1. **Open up a CLI**
Tis Docker-cli, so expect many CLI actions. 

2. **Install Packages**
Within your CLI enviroment, type "sudo apt install docker-compose"
...
Run the following command, "sudo usermod -aG docker \<user>" (see note 3.)
...
## Notes
1. Depending on your distro of choice(I use arch btw) you may not be able do anything, or commands may be different, such as debian based systems using "apt" and arch using "pacman".
...
3. Standard issue command to add a user to a group, man usermod for details if you wish.